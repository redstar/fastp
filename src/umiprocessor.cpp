...
void UmiProcessor::process(Read* r1, Read* r2) {
    ...
    if(mOptions->umi.location == UMI_LOC_INDEX1)
        ...
        umi = r1->mSeq.str().substr(0, min(r1->length(), mOptions->umi.length));
        r1->trimFront(umi.length() + mOptions->umi.skip);
    }
    else if(mOptions->umi.location == UMI_LOC_READ2 && r2){
        umi = r2->mSeq.str().substr(0, min(r2->length(), mOptions->umi.length));
        ...
        string umi1 = r1->mSeq.str().substr(0, min(r1->length(), mOptions->umi.length));
        string umiMerged = umi1;
        r1->trimFront(umi1.length() + mOptions->umi.skip);
        if(r2){
            string umi2 = r2->mSeq.str().substr(0, min(r2->length(), mOptions->umi.length));
